...
module.exports = async function(context, req) {
  ...
  try {
    ...
    let clients = [];
    try { const dl = await container.getBlockBlobClient(BLOB_NAME).downloadToBuffer(); clients = JSON.parse(dl.toString()); } catch(e) {}
    clients.sort((a,b) => a.business.localeCompare(b.business));
    context.res = {status:200,headers:CORS,body:JSON.stringify({clients})};
